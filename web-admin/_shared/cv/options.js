...
import {
  ...
  POSITION_LABELS, POSITION_SECTOR_LABELS, SKILL_LABELS, LICENSE_WORD_LABELS,
  EMPLOYMENT_STATUS_LABELS, WORK_AVAILABILITY_LABELS,
...
// Çalışma durumu: 'student' | 'employed'
export const EMPLOYMENT_STATUS = ['student', 'employed'];

// Çalışılabilir süre: sezonluk (3–6 ay) | tüm yıl (12 ay)
export const WORK_AVAILABILITY = ['seasonal', 'full_year'];

/** Eski 1–12 ay değerlerini yeni iki seçeneğe map eder. */
export function normalizeWorkAvailability(val) {
  if (val == null || val === '') return null;
  const s = String(val);
  if (s === 'full_year' || s === '12') return 'full_year';
  if (s === 'seasonal') return 'seasonal';
  const n = parseInt(s, 10);
  if (!Number.isNaN(n)) return n >= 12 ? 'full_year' : 'seasonal';
  return null;
}

// Çalışma alanı: önce sektör (Turizm / Diğer), sonra pozisyon.
// value sabit kalır — filtre ve eski kayıtlar bozulmaz.
export const POSITION_SECTORS = ['tourism', 'other'];

export const POSITIONS_BY_SECTOR = {
  tourism: [
    'Resepsiyon', 'Misafir İlişkileri Müdürü', 'Garson', 'Barmen', 'Barista',
    'Aşçı', 'Spor Animatörü', 'Çocuk Animatörü', 'Hostes',
    'Temizlik Görevlisi', 'Kat Görevlisi', 'Mutfak Yardımcısı', 'Masör / Masöz',
  ],
  other: ['Fabrika', 'İnşaat', 'Bahçe'],
};

export const POSITIONS = [
  ...POSITIONS_BY_SECTOR.tourism,
  ...POSITIONS_BY_SECTOR.other,
];

export function sectorOfPosition(value) {
  if (!value) return null;
  if (POSITIONS_BY_SECTOR.other.includes(value)) return 'other';
  if (POSITIONS_BY_SECTOR.tourism.includes(value)) return 'tourism';
  return 'tourism'; // bilinmeyen/eski değerler turizm altında sayılır
}
...
// Doğum yılları: 18–40 yaş aralığı (bugünün tarihine göre), yeni yıllar üstte
// Sınır yıllarda (tam 18 / tam 40) ay-gün kontrolü Step1'de yapılır (doğum günü geçti mi dahil).
...
// Mülakat: tek gün + 3 dilim. Saat/dakika serbest; dilim sınırları sabit.
// Dilim 0 sabah–öğle, 1 öğle–ikindi, 2 ikindi–akşam (en geç 23:59).
export const IV_BANDS = [
  { key: 'morning', min: '00:00', max: '11:59' },
  { key: 'noon', min: '12:00', max: '15:59' },
  { key: 'evening', min: '16:00', max: '23:59' },
];
// Eski sabit listeler (geriye dönük okuma / grup akışı için tutulur).
export const IV_MORNING = ['09:00', '09:30', '10:00', '10:30', '11:00', '11:30'];
export const IV_NOON = ['12:00', '12:30', '13:00', '13:30'];
export const IV_EVENING = ['14:00', '14:30', '15:00', '15:30', '16:00'];
export const INTERVIEW_TIMES = [...IV_MORNING, ...IV_NOON, ...IV_EVENING];

const _hhmmMin = (hhmm) => {
  const [h, m] = String(hhmm || '0:0').split(':').map(Number);
  return (h || 0) * 60 + (m || 0);
};
export function ivBandIndex(hhmm) {
  const mins = _hhmmMin(hhmm);
  for (let i = 0; i < IV_BANDS.length; i += 1) {
    if (mins >= _hhmmMin(IV_BANDS[i].min) && mins <= _hhmmMin(IV_BANDS[i].max)) return i;
  }
  return -1;
}
export function ivClampToBand(hhmm, bandIndex) {
  const b = IV_BANDS[bandIndex];
  if (!b) return hhmm;
  const mins = _hhmmMin(hhmm);
  if (mins < _hhmmMin(b.min)) return b.min;
  if (mins > _hhmmMin(b.max)) return b.max;
  const [h, m] = String(hhmm).split(':');
  return `${String(Number(h)).padStart(2, '0')}:${String(Number(m)).padStart(2, '0')}`;
}
export function ivDefaultInBand(bandIndex) {
  const defaults = ['09:00', '13:00', '17:00'];
  return defaults[bandIndex] || IV_BANDS[bandIndex]?.min || '09:00';
}

// Seçilen günde bu dilimde hâlâ gelecek bir dakika var mı? (yoksa dilim kilitlenir)
export function ivBandOpen(d, m, y, bandIndex, nowMs = Date.now()) {
  if (!(d && m && y)) return true; // gün seçilmeden hepsi açık görünür
  const b = IV_BANDS[bandIndex];
  if (!b) return false;
  const [H, M] = b.max.split(':').map(Number);
  const end = new Date(Number(y), Number(m) - 1, Number(d), H || 0, M || 0, 0, 0).getTime();
  return end > nowMs + 60 * 1000;
}

// Dilim içinde seçilebilecek en erken saat (şimdi + 1dk veya dilim başı).
export function ivEarliestInBand(d, m, y, bandIndex, nowMs = Date.now()) {
  const b = IV_BANDS[bandIndex];
  if (!b) return ivDefaultInBand(bandIndex);
  if (!(d && m && y)) return ivDefaultInBand(bandIndex);
  const [h0, m0] = b.min.split(':').map(Number);
  const bandStart = new Date(Number(y), Number(m) - 1, Number(d), h0 || 0, m0 || 0, 0, 0).getTime();
  const earliest = Math.max(bandStart, nowMs + 60 * 1000);
  const [H, M] = b.max.split(':').map(Number);
  const bandEnd = new Date(Number(y), Number(m) - 1, Number(d), H || 0, M || 0, 0, 0).getTime();
  if (earliest > bandEnd) return '';
  const dt = new Date(earliest);
  const pad = (n) => String(n).padStart(2, '0');
  return ivClampToBand(`${pad(dt.getHours())}:${pad(dt.getMinutes())}`, bandIndex);
}
...
export function langOptions(lang = 'tr') {
  return {
    ...
    POSITIONS_BY_SECTOR: {
      tourism: labelize(POSITIONS_BY_SECTOR.tourism, POSITION_LABELS, lang),
      other: labelize(POSITIONS_BY_SECTOR.other, POSITION_LABELS, lang),
    },
    POSITION_SECTORS: labelize(POSITION_SECTORS, POSITION_SECTOR_LABELS, lang),
    SKILLS: labelize(SKILLS, SKILL_LABELS, lang),
    EMPLOYMENT_STATUS: labelize(EMPLOYMENT_STATUS, EMPLOYMENT_STATUS_LABELS, lang),
    WORK_AVAILABILITY: labelize(WORK_AVAILABILITY, WORK_AVAILABILITY_LABELS, lang),
  };
}
