...
import { buildFlightHtml } from '../cv/buildFlightHtml';
import { withLatinName } from '../lib/translit';

const INK = '#1b2533';
const GOLD = '#c2a25a';

export default function FlightPreview({ visible, data, flight, onClose }) {
  const { t, dir } = useLanguage();
  const insets = useSafeAreaInsets();
  const backChevron = dir === 'rtl' ? '›' : '‹';
  const [sharing, setSharing] = useState(false);

  const html = useMemo(() => buildFlightHtml(withLatinName(data || {}), flight || {}), [data, flight]);
  ...
  const share = async () => {
    ...
    try {
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, { mimeType: 'application/pdf', dialogTitle: t('flight_info_title'), UTI: 'com.adobe.pdf' });
      }
    } catch (e) {
      Alert.alert(t('flight_info_title'), e?.message || 'error');
    } finally {
      setSharing(false);
    }
  };
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <View style={styles.wrap}>
        <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
          ...
          <Text style={styles.title} numberOfLines={1}>{t('flight_info_title')}</Text>
          <View style={{ width: 28 }} />
        </View>
