// Web e-imza yardımcıları — mobil lib/esign ile AYNI mantık (saf JS; native yok).
// SHA-256 (belge bütünlüğü) + denetim satırı. Supabase'e yazma api.js'te.
...
function sha256(bytes) {
  ...
  const w8 = bytes.slice(); w8.push(0x80);
  while (w8.length % 64 !== 56) w8.push(0);
  const bitLen = l * 8;
  for (let i = 7; i >= 0; i--) w8.push((bitLen / Math.pow(2, i * 8)) & 0xff);
  const rotr = (x, n) => (x >>> n) | (x << (32 - n));
  const w = new Array(64);
  for (let off = 0; off < w8.length; off += 64) {
    for (let i = 0; i < 16; i++) w[i] = (w8[off+i*4] << 24) | (w8[off+i*4+1] << 16) | (w8[off+i*4+2] << 8) | (w8[off+i*4+3]);
    for (let i = 16; i < 64; i++) {
