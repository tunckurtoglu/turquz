...
export default function ContractPortal() {
  ...
  const printContract = async () => {
    ...
    if (!w) { setErr('Açılır pencere engellendi. Lütfen izin verin.'); return; }
  ...
  const onDownload = async () => {
    ...
    try {
      ...
      if (st.paid) { await printContract(); return; }
      const c = await portalCall({ action: 'checkout', token });
      if (c.paid) { setPaid(true); await printContract(); return; }
      if (c.checkoutUrl) { window.location.href = c.checkoutUrl; return; }
