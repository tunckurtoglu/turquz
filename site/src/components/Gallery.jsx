...
export default function Gallery() {
  ...
  // Otomatik geçiş (tek görsel veya boşsa durur).
  useEffect(() => {
    if (n < 2) return;
  ...
  // RTL'de ok yönlerini görsel olarak tutarlı tut.
        ...
        <Reveal className="gallery">
          <div className="slides">
            {has ? (
              ...
              <div className="slides__empty">
                <div>
                  <div className="ph-ic"><IconSpark size={64} /></div>
                  <p>{t('gal.empty')}</p>
                  <p style={{ marginTop: 10, fontSize: '0.85rem', opacity: 0.7 }}>
                    <code>site/src/gallery/</code>
                  </p>
                </div>
              </div>
            )}
            ...
            {n > 1 && (
              <>
                <button className="slide-nav prev" onClick={() => move(prevStep)} aria-label="prev" style={{ transform: 'translateY(-50%) rotate(90deg)' }}>
                  <IconChevron size={22} />
                </button>
                <button className="slide-nav next" onClick={() => move(-prevStep)} aria-label="next" style={{ transform: 'translateY(-50%) rotate(-90deg)' }}>
                  <IconChevron size={22} />
                </button>
              </>
            )}
