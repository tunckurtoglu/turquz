...
/** Acentenin favoriye aldığı tüm aday id'leri (benzersiz). */
export async function listAllFavoritedCandidateIds(agencyId) {
  if (!agencyId) return [];
  const { data, error } = await supabase
    .from('agency_favorites')
    .select('candidate_id')
    .eq('agency_id', agencyId);
  if (error) {
    console.warn('all favorite ids:', error.message);
    return [];
  }
  return [...new Set((data || []).map((r) => r.candidate_id).filter(Boolean))];
}

/** Teklif öncesi favori zorunluluğu. */
export async function hasFavoriteSlot(agencyId, candidateId) {
