...
export const IV_RESPOND_MS = 48 * 60 * 60 * 1000;

export function interviewRespondDeadlineMs(iv) {
  if (!iv) return 0;
  const respondBy = iv.respond_by || iv.ivRespondBy;
  if (respondBy) return new Date(respondBy).getTime();
  const base = iv.created_at || iv.ivCreatedAt || iv.updated_at;
  if (!base) return 0;
  return new Date(base).getTime() + IV_RESPOND_MS;
}
