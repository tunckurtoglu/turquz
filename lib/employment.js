...
export async function scanEmploymentLifecycle() {
  try {
    ...
    try {
      ...
      console.warn('Havaalanı teyit taraması:', e?.message || e);
    }
    try {
...
export async function answerAirportCheck(answer /* 'confirmed' | 'missed' */) {
  const { error } = await supabase.rpc('candidate_answer_airport_check', { p_answer: answer });
  if (error) throw error;
}
