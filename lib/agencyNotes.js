...
async function writeLocal(key, rows) {
  await AsyncStorage.setItem(key, JSON.stringify((rows || []).slice(0, 40)));
}
...
export async function listDeskNotes(agencyId) {
  ...
  if (!error) {
    const rows = data || [];
    writeLocal(deskKey(agencyId), rows).catch(() => {});
    return rows;
  }
...
export async function addDeskNote(agencyId, body) {
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (!error && data) {
    const prev = await readLocal(deskKey(agencyId));
    writeLocal(deskKey(agencyId), [data, ...prev.filter((n) => n.id !== data.id)]).catch(() => {});
    return data;
  }
  if (error) console.warn('desk note insert:', error.message);
  const row = localRow(text);
  const prev = await readLocal(deskKey(agencyId));
  await writeLocal(deskKey(agencyId), [row, ...prev]);
  return row;
}
...
export async function removeDeskNote(agencyId, noteId) {
  ...
  const prev = await readLocal(deskKey(agencyId));
  writeLocal(deskKey(agencyId), prev.filter((n) => n.id !== noteId)).catch(() => {});
...
export async function addEmployerNote(agencyId, employerId, body) {
  ...
  if (!error && data) {
    ...
    writeLocal(key, [data, ...prev.filter((n) => n.id !== data.id)]).catch(() => {});
    return data;
  }
  if (error) console.warn('employer note insert:', error.message);
  const row = localRow(text);
  const prev = await readLocal(key);
  await writeLocal(key, [row, ...prev]);
  return row;
}
...
export async function removeEmployerNote(agencyId, employerId, noteId) {
  ...
  const key = empKey(agencyId, employerId);
  const prev = await readLocal(key);
  writeLocal(key, prev.filter((n) => n.id !== noteId)).catch(() => {});
  if (String(noteId).startsWith('local-')) return;
  const { error } = await supabase
    .from('agency_employer_notes')
    .delete()
