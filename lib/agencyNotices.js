...
import { activeStep } from './pipeline';
...
export const NOTICE_BUCKETS = [
  ...
  { id: 'pool', labelKey: 'agency_notice_aud_pool' },
...
function photoOf(data) {
  const d = data || {};
  return d.photoClose || d.photo || d.photoFull || '';
}
...
export async function listNoticeAudienceBuckets(agencyId) {
  ...
  const [pool, offered, process, hired, transit, ivs, favs] = await Promise.all([
    fetchPaged('candidate_pool', 'user_id'),
    aid
  ...
  ].map((r) => r.user_id).concat((favs || []).map((r) => r.candidate_id));
  ...
  (pool || []).forEach((p) => buckets.pool.push({ userId: p.user_id, code: '', nationality: '', title: '', name: '', photo: '' }));
  (favs || []).forEach((r) => add('fav', r.candidate_id));
